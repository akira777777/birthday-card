...
// Use 3 gnomes matching our PNG assets
const GNOME_COUNT = 3
...
export default function BirthdayCardPage() {
  ...
  const clicksNeeded = useMemo(() => (isMobile ? 4 : 6), [isMobile])
  ...
  const generateRandomPosition = useCallback(
    (index: number): GnomePosition => {
      ...
      const gnomeSize = isMobile ? 100 : 140
      const padding = isMobile ? 60 : Math.max(150, window.innerWidth * 0.1)
      const topPadding = isMobile ? 140 : padding + 20
      const bottomPadding = isMobile ? 120 : padding + 20
  ...
  const handleGnomeClick = useCallback(
    (clickX: number, clickY: number, gnomeId?: number) => {
      ...
      if (fireworksRef.current) {
        fireworksRef.current.launch(clickX, clickY, isMobile ? 40 : 60)
      }
      ...
      if (newCount >= clicksNeeded) {
        ...
        trackTimeout(setTimeout(() => {
          if (fireworksRef.current) {
            ...
            const extraParticles = isMobile ? 50 : 90
            ...
            const additionalCount = isMobile ? 4 : 8
            ...
            trackTimeout(setTimeout(
              () => {
                if (fireworksRef.current) {
                  fireworksRef.current.launch(centerX, centerY, isMobile ? 120 : 180)
                  trackTimeout(setTimeout(() => {
                    if (fireworksRef.current) {
                ...
                setShowCard(true)
                // Activate confetti after card appears
                trackTimeout(setTimeout(() => {
        ...
        // Move clicked gnome to new position with smooth transition
        trackTimeout(setTimeout(() => {
          setGnomes((prev) => {
  ...
  const starsCount = isMobile ? 30 : 60
  const twinkleCount = isMobile ? 8 : 16
    ...
    <div className="fixed inset-0 overflow-hidden bg-gradient-to-br from-purple-900 via-indigo-900 via-blue-900 to-purple-950 touch-none select-none">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        {twinkles.map((twinkle, i) => (
          <div
            key={`twinkle-${i}`}
            className="absolute rounded-full bg-yellow-200 animate-twinkle-star"
            style={{
              left: `${twinkle.left}%`,
              top: `${twinkle.top}%`,
              ...
              boxShadow: "0 0 8px rgba(255, 255, 200, 1)",
            }}
          />
        ))}
      </div>
      ...
      {!showCard && gnomesVisible && (
        <div
          className={`absolute left-1/2 transform -translate-x-1/2 z-20 bg-black/70 backdrop-blur-xl border-2 border-yellow-400/80 shadow-2xl shadow-yellow-400/30 animate-slide-down ${isMobile ? "top-4 rounded-2xl px-4 py-3" : "top-8 rounded-full px-8 py-4"
            }`}
        >
          <div className={`flex ${isMobile ? "flex-col gap-2" : "flex-row items-center gap-4"}`}>
            <p className={`text-white font-bold whitespace-nowrap text-center ${isMobile ? "text-base" : "text-lg"}`}>
              Найдено гномиков: <span className="text-yellow-400 animate-pulse-soft">{clickCount}</span> / {clicksNeeded}
            </p>
            {/* Progress bar */}
            <div
              className={`bg-black/40 rounded-full overflow-hidden border border-yellow-400/30 ${isMobile ? "w-full h-2.5" : "w-48 h-3"
                }`}
            >
              <div
                className="h-full bg-gradient-to-r from-yellow-400 via-yellow-300 to-yellow-500 rounded-full transition-all duration-500 ease-out relative overflow-hidden"
                style={{ width: `${(clickCount / clicksNeeded) * 100}%` }}
              >
                <div
                  className="absolute inset-0 bg-gradient-to-r from-transparent via-white/40 to-transparent animate-shimmer-smooth"
      ...
      {clickCount === 0 && !showCard && (
        <div
          className={`absolute left-1/2 transform -translate-x-1/2 z-20 bg-black/70 backdrop-blur-xl border-2 border-yellow-400/80 shadow-2xl shadow-yellow-400/30 animate-fade-up ${isMobile ? "bottom-6 rounded-2xl px-5 py-3 mx-4 max-w-[calc(100%-2rem)]" : "bottom-8 rounded-2xl px-10 py-5"
            }`}
        >
          <p
          ...
          >
            <span className={`animate-bounce-gentle ${isMobile ? "text-xl" : "text-2xl md:text-3xl"}`}>🎯</span>
            <span>{isMobile ? "Нажми на гномиков!" : "Найди и кликни на всех гномиков!"}</span>
            <span
              className={`animate-bounce-gentle ${isMobile ? "text-xl" : "text-2xl md:text-3xl"}`}
