...
function getGnomeBox(isMobile: boolean) {
  return { width: isMobile ? 92 : 120, height: isMobile ? 132 : 175 }
}
...
export default function BirthdayCardPage() {
  ...
  const generateRandomPosition = useCallback(
    (index: number): GnomePosition => {
      ...
      const box = getGnomeBox(isMobile)
      const padding = isMobile ? 40 : Math.max(90, window.innerWidth * 0.08)
      const topPadding = isMobile ? 110 : padding // Space for progress
      const bottomPadding = isMobile ? 110 : padding // Space for hint

      const maxX = window.innerWidth - padding - box.width
      const maxY = window.innerHeight - bottomPadding - box.height
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-black touch-none select-none">
      {/* Safe area padding for mobile notch/home indicator */}
      <div className="absolute inset-0 safe-area-inset" />

      {/* PNG-based background (card image, blurred + dimmed via filter) */}
      ...
      <BirthdayCard
        ...
        isMobile={isMobile}
        onReplay={() => {
          // Reset state for another round
          setClickCount(0)
          setShowCard(false)
          setGnomesVisible(true)
          setConfettiActive(false)
          setGnomes(Array.from({ length: GNOME_COUNT }, (_, i) => generateRandomPosition(i)))
      ...
      {clickCount === 0 && !showCard && (
        ...
        >
          <p className={`text-white font-bold text-center ${isMobile ? "text-base" : "text-xl md:text-2xl"}`}>
            {isMobile ? "Нажми на гномиков!" : "Найди и кликни на всех гномиков!"}
          </p>
        </div>
      )}
    </div>
